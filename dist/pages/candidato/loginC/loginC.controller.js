"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LoginCController = void 0;
const common_1 = require("@nestjs/common");
const loginC_service_1 = require("./loginC.service");
let LoginCController = class LoginCController {
    constructor(loginCService) {
        this.loginCService = loginCService;
    }
    async verificarLogin(rm, dataNascimento) {
        return this.loginCService.verificarLogin(rm, dataNascimento);
    }
    async atualizarCadastro(rm, dados) {
        return this.loginCService.atualizarCadastro(rm, dados);
    }
    async buscarCandidato(rm) {
        console.log('Buscando candidato com RM:', rm);
        const candidato = await this.loginCService.buscarCandidatoPorRm(rm);
        if (!candidato) {
            throw new common_1.NotFoundException('Candidato não encontrado');
        }
        return candidato;
    }
};
exports.LoginCController = LoginCController;
__decorate([
    (0, common_1.Post)('verificar'),
    __param(0, (0, common_1.Body)('rm')),
    __param(1, (0, common_1.Body)('dataNascimento')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], LoginCController.prototype, "verificarLogin", null);
__decorate([
    (0, common_1.Post)('atualizar'),
    __param(0, (0, common_1.Body)('rm')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], LoginCController.prototype, "atualizarCadastro", null);
__decorate([
    (0, common_1.Get)('candidato/:rm'),
    __param(0, (0, common_1.Param)('rm')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], LoginCController.prototype, "buscarCandidato", null);
exports.LoginCController = LoginCController = __decorate([
    (0, common_1.Controller)('login'),
    __metadata("design:paramtypes", [loginC_service_1.LoginCService])
], LoginCController);
//# sourceMappingURL=loginC.controller.js.map